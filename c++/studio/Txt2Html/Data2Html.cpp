...
int Data2Html::GetData(wxString& word, wxString& html)
{
	if(!_ifs.is_open())
		return -1;
...
int Data2Html::Make(const TxtTidy::TData &data, wxString& word, wxString& html) const
{
	...
	word = wxString(data.word.c_str(), wxConvUTF8);

	html = STR_1;
	html += _dict;
	
	html += STR_2;
	html +=  wxString(data.word.c_str(), wxConvUTF8);
	html += STR_3;

	html += wxString(data.symbol.c_str(), wxConvUTF8);
	html += STR_4;

	for(std::vector<std::pair<std::string, std::string> >::const_iterator it = data.data.begin(); it != data.data.end(); ++ it)
	{
		html += STR_5;
		html += wxString(it->first.c_str(), wxConvUTF8);
		html += STR_6;
		html += wxString(it->second.c_str(), wxConvUTF8);
		html += STR_7;
	}
