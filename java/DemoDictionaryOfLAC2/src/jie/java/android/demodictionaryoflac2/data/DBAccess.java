...
//				+ " where ref_index_" + dictid + ".wordid = ? and word_index_" + dictid + ".idx = ref_index_" + dictid + ".ref_idx", new String[] { String.valueOf(wordid) });
		...
		return db.rawQuery("select offset, length, block1 from word_index_" + dictid + " inner join ref_index_" + dictid + " on ((word_index_" + dictid + ".idx = ref_index_" + dictid + ".ref_idx)) where (ref_index_" + dictid + ".wordid=?)", new String[] { String.valueOf(wordid) });
