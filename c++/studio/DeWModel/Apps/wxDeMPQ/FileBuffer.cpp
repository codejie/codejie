...
int CFileBuffer::Attach(const std::string& filename)
{
	...
	_stFileStream.open(filename.c_str(), std::ios::in | std::ios::binary);
...
int CFileBuffer::Skip(size_t offset)
{
	_stFileStream.seekg(offset, std::ios_base::cur);
	if(!_stFileStream.good())
		return -1;
	return 0;
}
