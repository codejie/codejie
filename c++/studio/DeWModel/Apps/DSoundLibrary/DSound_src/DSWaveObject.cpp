...
double CDSWaveObject::Duration()
{
	...
	//	return 0;
	//WAVEFORMATEX wformat;
	//HRESULT hr = _pDSBuffer->GetFormat(&wformat, sizeof(WAVEFORMATEX), NULL);
	//if(hr != DS_OK)
	//	return 0;
	...
	//hr = _pDSBuffer->GetCaps(&caps);
	//if(hr != DS_OK)
	//	return -1;

	//return (caps.dwBufferBytes * 1000 / wformat.nAvgBytesPerSec);
}

int CDSWaveObject::SetPlayPos(unsigned int pos)
{
	_uiReadSize = pos + sizeof(WAVE_HEADER);
	if(_uiReadSize > _headerWave.data_size)
		return -1;
