    ...
    wxFrame(parent, id, title, pos, size, wxCAPTION|wxCLOSE_BOX|wxSYSTEM_MENU)
{
	...
	m_editOffset = new wxTextCtrl(this, wxID_ANY, wxEmptyString);
...
void CDSoundFrame::OnBtnPlay(wxCommandEvent &event)
{
    ...
    event.Skip();
    wxLogDebug(wxT("Event handler (CDSoundFrame::OnBtnPlay) not implemented yet")); //notify the user that he hasn't implemented the event handler yet
}
...
//	vol = 85000;
//	obj->SetPlayPos(vol);
////	size_t t = obj->Duration();
	long offset = 0;
	m_editOffset->GetValue().ToLong(&offset);
	obj->SetPlayPos(offset);
    event.Skip();
    wxLogDebug(wxT("Event handler (CDSoundFrame::OnBtnStop) not implemented yet")); //notify the user that he hasn't implemented the event handler yet
}
...
void CDSoundFrame::do_layout()
{
	...
	sizer_3->Add(m_editOffset, 0, wxLEFT|wxRIGHT|wxEXPAND|wxALIGN_CENTER_VERTICAL, 4);
