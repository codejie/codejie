...
DTUConnectorApp theApp;

#include "DTUConnectorApp.h"


DTUConnectorApp::DTUConnectorApp()
: ACEF_App()
...
int DTUConnectorApp::init(int argc, char* argv[])
{
...
int DTUConnectorApp::regist_signal()
{
...
int DTUConnectorApp::regist_command()
{
...
int DTUConnectorApp::handle_signal(int signum, siginfo_t* siginfo, ucontext_t* ucontext)
{
	ACEX_LOG_OS(LM_WARNING, "<<DTUConnectorApp::handle_signal>>Recv signal '" << signum << "',");
	if(signum != 2)//SIGPIPE
	{
...
int DTUConnectorApp::handle_msg(const ACEX_Message& msg)
{
...
int DTUConnectorApp::InitConfig()
{
...
int DTUConnectorApp::StartTasks()
{
	try
	{
...
void DTUConnectorApp::StopTasks()
{
	if(!_stOptions.DaemonOpt())
	{
...
void DTUConnectorApp::ShowVersion(std::ostream& os) const
{
	os << "\n--------------------------------------------------";
...
void DTUConnectorApp::ShowOptions(std::ostream& os) const
{
	_stOptions.Show(os);
}

void DTUConnectorApp::ShowConfig(std::ostream& os) const
{
	_stConfigLoader.Show(os);
}
//
//void DTUConnectorApp::ShowData(bool stat, std::ostream& os) const
//{
//    _taskCore.ShowData(stat, os);
//}
//
//void DTUConnectorApp::ShowPacket(std::ostream& os) const
//{
//    _taskCore.ShowPacket(os);
//}
//
//void DTUConnectorApp::ShowStationID(std::ostream& os, const std::string& ano) const
//{
//    _taskCore.ShowStationID(os, ano);
//}
//
//void DTUConnectorApp::ShowInfectantID(std::ostream& os, const std::string& nid) const
//{
//    _taskCore.ShowInfectantID(os, nid);
//}
//
//void DTUConnectorApp::ShowTerminal(std::ostream& os) const
//{
//	_taskCore.ShowClient(os);
//}

int DTUConnectorApp::Shutdown()
{
...
int DTUConnectorApp::OnAppTaskMsgProc(const ACEX_Message& msg)
{
	if(msg.fparam() == FPARAM_SHUTDOWN)
	{
		ACEX_LOG_OS(LM_INFO, "<<DTUConnectorApp::OnAppTaskMsgProc>>Recv 'shutdown' msg." << std::endl);
		return -1;
	}
	else
	{
		ACEX_LOG_OS(LM_WARNING, "<<DTUConnectorApp::OnAppTaskMsgProc>>Unknwon fparam - " << msg.fparam() << std::endl);
	}
	return 0;
}

const std::string DTUConnectorApp::TimeToString(const time_t& tTime) const
{
	char acTime[19 + 1];
	acTime[19] = '\0';
	struct tm tmTime;
